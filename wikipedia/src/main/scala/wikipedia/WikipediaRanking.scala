...
object WikipediaRanking extends WikipediaRankingInterface {
  ...
  def occurrencesOfLang(lang: String, rdd: RDD[WikipediaArticle]): Int = {
    rdd.map(article => article.mentionsLanguage(lang)).aggregate(0)((U, b) => {if (b) U+1 else U}, _+_)
  }
//  def occurrencesOfLang(lang: String, rdd: RDD[WikipediaArticle]): Int = {
...
//  }).sortWith(_._2 > _._2)
  ...
  def makeIndex(langs: List[String], rdd: RDD[WikipediaArticle]): RDD[(String, Iterable[WikipediaArticle])] = rdd
    .flatMap(a => langs.filter(l => a.mentionsLanguage(l)).map((_, a))).groupByKey()
  ...
  def rankLangsReduceByKey(langs: List[String], rdd: RDD[WikipediaArticle]): List[(String, Int)] =
    rdd.flatMap(a => langs.filter(l => a.mentionsLanguage(l)).map((_, 1))).reduceByKey(_+_).sortBy(-_._2).collect().toList
