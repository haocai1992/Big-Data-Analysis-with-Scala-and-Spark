 ...
 */
 class ListsSuite {
  ...
  @Test def `sum of a few numbers-1 (10pts)`: Unit = {
    assert(sum(List(0,0,0)) == 0)
  }

  @Test def `sum of a few numbers-2 (10pts)`: Unit = {
  ...
  @Test def `max of a few numbers-1 (10pts)`: Unit = {
    assert(max(List(3, -7, 2)) == 3)
  }

  @Test def `max of a few numbers-2 (10pts)`: Unit = {
